...
/**
 * Where a person flashes a blank board: flasher/ in the Pages stand of this
 * repo, built by .github/workflows/pages.yml. The root of that stand is the
 * handbook, which points here from its front page - so a release note written
 * before the flasher moved (2026-09-23) still leads a person to it. Named here
 * because everything that points at it imports this module anyway - the
 * designer's firmware section and, from the firmware repo, the release tool
 * that writes it into every release note.
 */
export const FLASHER_URL = "https://matthias-hess.github.io/schaltli-designer/flasher/"
 ...
 * certainly can, and every Schaltli board is an ESP32-S3. An image built for
...
export function refuseChip(description) {
  ...
  return `this is an ${text.split("(")[0].trim() || "unknown chip"}, and every Schaltli board is an ESP32-S3` +
